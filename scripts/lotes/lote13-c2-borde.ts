...
export const PARES: ParMinimo[] = [
  {
    id: 'P-01', concepto: C,
    rasgo: 'el español duplica el clítico con un SN dativo explícito; el portugués europeo no',
    esqueleto: 'O advogado {} ao director que a proposta seguia por correio registado.',
    bien: 'disse', mal: 'disse-lhe',
    glosaBien: 'El abogado dijo al director que la propuesta seguía por correo certificado.',
    glosaMal: 'El abogado le dijo al director que la propuesta seguía por correo certificado.',
    explicacionBien:
      'Con el destinatario expreso —«ao director»— el portugués europeo NO ' +
      'pone además el clítico. El español sí lo duplica («le dijo al ' +
      'director») y lo duplica tanto que suena mal sin él: por eso el calco ' +
      'no se nota al traducir.',
    explicacionMal:
      'Es la duplicación española trasplantada. «Disse-lhe ao director» pone ' +
      'dos veces el mismo complemento indirecto: el clítico «-lhe» y el ' +
      'sintagma «ao director». En portugués europeo se elige uno de los dos.',
  },
  {
    id: 'P-02', concepto: C,
    ...
    bien: 'o', mal: 'ao',
    glosaBien: 'Fuimos a visitar el tu abuelo al asilo, pero él ya estaba durmiendo.',
    glosaMal: 'Fuimos a visitar al tu abuelo al asilo, pero él ya estaba durmiendo.',
    ...
    explicacionBien:
      ...
      'El portugués no tiene la «a» personal del español, y ésta es la que ' +
      'más resiste porque en español es obligatoria.',
    explicacionMal:
      'La «a» personal española, trasplantada. «Visitar ao teu avô» sólo ' +
      'sería posible si «ao» fuera el régimen del verbo, y «visitar» es ' +
      'transitivo directo.',
  },
  {
    id: 'P-03', concepto: C,
    ...
    explicacionMal:
      '«Atrás meu» calca el «detrás mío» español, que en español está ' +
      'extendido —hasta el punto de sonar natural— y en portugués no existe ' +
      'en ninguna variedad.',
  },
];
...
for (const p of PARES) {
  ...
  GLOSAS[rellenar(p.esqueleto, p.bien)] = { es: p.glosaBien!, ok: buena };
  GLOSAS[rellenar(p.esqueleto, p.mal)] = { es: p.glosaMal!, ok: buena };
}
...
L.push('');
L.push('**Sesión E2#14.** Seis ítems, **no doce**, y por decisión: la regla de');
L.push('corte de Edu. Tres pares defendibles valen más que seis inventados, y ya');
L.push('van **siete MAL muertos en tres sesiones** por cruzar esa frontera. Los');
L.push('otros tres pares quedan parados, con sus descartes razonados, en');
L.push('`2026-09-04-banco-c2-borde-gramaticalidad.md`.');
L.push('');
L.push('| punto | antes | del lote | tras el lote |');
L.push('|---|---:|---:|---:|');
L.push(`| \`${C}\` | **0** | 6 | **6** — no cierra, y se declara |`);
L.push('');
...
L.push('igual de mal). Aquí: P-01 y P-03 glosan bien en sus dos miembros, P-02');
L.push('glosa mal en los dos. El rasgo vale lo mismo dentro de cada par y queda');
L.push('neutro **por teorema**, no por suerte.');
L.push('');
L.push('## Los tres pares, verificados contra el corpus');
L.push('');
