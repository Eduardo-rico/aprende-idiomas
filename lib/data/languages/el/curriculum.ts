// lib/data/languages/el/curriculum.ts
// Fase H (2026-09-13): scaffold vacío para el griego MODERNO. El
// currículo no existe todavía —es el eslabón que RO sí tenía y estas dos
// no (ver docs/plans/2026-09-13-el-arranque.md)—, así que aquí no
...
export function getBlock(_id: number): Block {
  throw new Error("No blocks for grc yet (fase G: el currículo aún no está escrito).");
}

export function getLesson(_id: LessonId): Lesson {
  throw new Error("No lessons for grc yet (fase G: el currículo aún no está escrito).");
}
