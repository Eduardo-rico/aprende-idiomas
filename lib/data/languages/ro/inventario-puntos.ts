...
export const PUNTOS_RO: PuntoRo[] = [
  ...
  P({ id: 'r2-numerales-de', nombre: 'Numerales y la regla del «de» desde 20', bloque: 2, nivel: 'A1',
    ...
    varianza: 'INSTANCIA REAL — DEFECTO, pendiente de arreglo. Pone «de» en los 8 y los OCHO numerales son ≥20 (20, 25, 30, 40, 50, 80, 100, 200): la regla es «≥20 pide de, <20 no», y la mitad negativa no se examina NUNCA. Un alumno que aprenda «pon de detrás del numeral» saca 8/8 y luego escribe *cinci de mere. Lo que varía —qué numeral— no cambia la respuesta, así que no discrimina. ARREGLO, y es barato porque el formato SÍ lo admite: un ítem cuyo error sea la SOBREaplicación, *Am cinci de mere → Am cinci mere, igual que hace r4-gd-lui-formula con su octavo.',
  ...
  P({ id: 'r6-pe-regla-operativa', nombre: 'Marcador «pe» de objeto directo', bloque: 6, nivel: 'A1',
    ...
    varianza: 'INSTANCIA REAL — DEFECTO, Y LO CREÓ LA RETIRADA DEL 2026-09-03. Quita «pe» en los 4 que quedan, todos del mismo lado de la regla (indefinido no específico: caut un doctor). Los cuatro que enseñaban el lado contrario —«pe» SÍ obligatorio— eran las relativas retiradas, así que el punto no sólo bajó de 8 a 4: quedó DESEQUILIBRADO, enseñando media regla. Consecuencia operativa para quien pague la deuda: las 4 unidades que faltan tienen que ser del lado «sí va pe» (pronombres pe cineva/pe nimeni/pe cine, persona individualizada), no más ítems de quitar «pe».',
  ...
  P({ id: 'r8-relativas-pe-care', nombre: 'Relativas de OD: «pe care» + reluare clitică, las DOS piezas a la vez', bloque: 8, nivel: 'B1',
    ...
    varianza: 'PARCIAL — la varianza está resuelta, el caso negativo no. Pone «pe» en los 8, y lo que varía es el clítico îl/o/îi/le. Eso ERA el defecto (la varianza pertenecía a r6-cliticos-acusativo) y está resuelto por las dos vías que el lingüista pidió: el punto se RENOMBRÓ para que el paquete «pe care + reluare clitică» sea el punto —así la concordancia es suya y no de r6— y los ocho ítems se pasaron a PRESENTE, que elimina la elisión ante auxiliar y la enclisis del participio, dos subdestrezas ajenas. LO QUE SIGUE FALTANDO: ningún ítem muestra la relativa de SUJETO (omul care vine), donde «pe» NO va. Un alumno que aprenda «pon pe care» escribirá *omul pe care vine. ARREGLO: un ítem de sobreaplicación.',
