...
export interface PuntoRo {
  ...
  pisoCero?: string;
  /** EL RASGO DIANA NO VARÍA ENTRE LOS ÍTEMS, y aquí está el juicio de si
   *  eso es un defecto o no. Lo marca `scripts/lib/varianza.ts`: una pieza
   *  de la operación presente en ≥80 % de los ítems se aprende en el
   *  primero y a partir del segundo no discrimina a nadie.
   *
   *  La señal es NECESARIA Y NO SUFICIENTE, y por eso este campo es prosa
   *  y no un booleano: la pregunta que decide es **qué varía en su lugar y
   *  si eso pertenece a este punto**. `r3-negacion-antepuesta` pone «nu» en
   *  los ocho porque en rumano no hay palabra negativa que no lo exija —
   *  la invariancia es propiedad de la LENGUA— mientras que
   *  `r2-numerales-de` pone «de» en los ocho porque los ocho numerales son
   *  ≥20 y la mitad de la regla no se examina nunca.
   *
   *  Nace de `r8-relativas-pe-care`: ocho ítems impecables uno por uno que
   *  eran UN ítem repetido ocho veces. No lo ve ningún gate por ítem
   *  porque no es propiedad de ningún ítem. El invariante no es un número:
   *  es «cero puntos marcados sin juicio escrito», la forma del `pisoCero`
   *  y la de la cuarentena. */
  varianza?: string;
...
export const PUNTOS_RO: PuntoRo[] = [
  ...
  P({ id: 'r2-genero-tres-valores', nombre: 'Género de tres valores: la prueba un…/două…', bloque: 2, nivel: 'A1',
    ...
    varianza: 'LEGÍTIMO, y por la misma razón que r3-negacion-antepuesta: la invariancia es de la LENGUA. Pone «un» en los 8 porque el rumano NO TIENE otro artículo indefinido para masculino ni para neutro, y que las dos casillas se escriban igual en singular es exactamente por lo que este punto existe — el ambigen se define por distribución y no por forma (GALR I, Substantivul · Genul; GBLR 2010), así que NINGUNA casilla del singular puede discriminar masculino de neutro y el punto vive entero en el plural. Lo que varía en su lugar, y ES el punto, es el género del sustantivo: cuatro masculinos que piden «doi» y cuatro neutros que piden «două», sin ninguna pista en la frase que lo diga. RESERVA ESCRITA (a): la invariante «un» 8/8 no es neutra — además de «el singular no discrimina», el lote enseña «sólo hay un artículo indefinido», que es falso. Es el precio de excluir el femenino, que es forzoso por aritmética (con un solo femenino dentro, «si la fuente lleva un di doi» acierta masculinos y femeninos y pasa el tope del 50 %), y el ANTÍDOTO es r2-articulo-indefinido, que lleva «o» y «niște» con ocho ítems: no cuenta como cobertura de este punto sino como reparación de esa falsedad. RESERVA ESCRITA (b): la concordancia del adjetivo viaja en los ocho como segundo exponente del mismo rasgo, pero está publicada entera en r2-concordancia-adjetivo y NO es variable de este punto — quien acierte el numeral y falle el adjetivo falla aquél. RESERVA ESCRITA (c): r2-numerales-de declara «doi/două concuerda en género» en su propia descripción y el lote 25 lo usaba de relleno; en los dos el numeral es RUIDO y aquí es la variable examinada. Queda escrito para que no se lea como duplicado.' }),
  ...
  P({ id: 'r2-numerales-de', nombre: 'Numerales y la regla del «de» desde 20', bloque: 2, nivel: 'A1',
    ...
    varianza: 'ARREGLADO EL 2026-09-03 (lote 20), y se conserva el diagnóstico porque el punto sigue marcado y el juicio tiene que explicar por qué ya no es defecto. ERA: ponía «de» en los 8 y los OCHO numerales son ≥20 (20, 25, 30, 40, 50, 80, 100, 200): la regla es «≥20 pide de, <20 no», y la mitad negativa no se examina NUNCA. Un alumno que aprenda «pon de detrás del numeral» saca 8/8 y luego escribe *cinci de mere. Lo que varía —qué numeral— no cambia la respuesta, así que no discrimina. ARREGLADO con dos ítems de SOBREaplicación (*cinci de mere, *nouăsprezece de elevi), igual que hace r4-gd-lui-formula con su octavo. La frontera 19/20 está verificada como normativa: hasta 19 el numeral determina al nombre directamente y a partir de 20 el nombre se subordina con «de» (limbaromana.net, Numeralul cardinal; dictie.ro; NO se pudo citar DOOM3, que no publica el artículo del numeral en abierto). GATE: el nombre contado va en ALLOWLIST, porque en coloquial existe «cinci de-alea» (numeral + «de» + demostrativo popular) donde ese «de» es OTRO morfema y la mala sería lengua real. PENDIENTE, con fuente NO verificada y por eso no aplicado: el lingüista avisa de que en uso identificativo el numeral ≥20 iría SIN «de» (pagina douăzeci și cinci, ora douăzeci); se revisaron los ocho publicados y NINGUNO usa esa estructura —todos cuentan— así que no hay nada que arreglar, pero quien añada ítems aquí tiene que comprobarlo.',
  ...
  P({ id: 'r3-trebuie-invariable', nombre: '«a trebui» invariable: trebuie să merg / trebuie să mergem', bloque: 3, nivel: 'A1',
    ...
    varianza: 'LEGÍTIMO. Pone «trebuie» en los 8, pero lo que VARÍA es la forma conjugada que QUITA —trebuim, trebuiți, trebuiesc, trebuiești, sobre 4 personas—, y ésa es exactamente la cara del punto: la regla es «da igual la persona, siempre trebuie», así que el alumno tiene que reconocer el calco en cada persona. La respuesta constante no es el rasgo diana; el rasgo diana es la persona del error. RESERVA ESCRITA: no hay ítem del caso donde «a trebui» SÍ flexiona (trebuia, va trebui), así que el punto enseña la invariabilidad del PRESENTE como si fuera del verbo entero.',
  ...
  P({ id: 'r3-negacion-antepuesta', nombre: 'El negativo ANTEPUESTO conserva «nu»: niciodată nu, nimeni nu, nimic nu', bloque: 3, nivel: 'A1',
    ...
    varianza: 'LEGÍTIMO, y es el caso que demuestra que la señal no basta. Pone «nu» en los 8 porque EN RUMANO NO HAY PALABRA NEGATIVA QUE NO LO EXIJA: la invariancia es propiedad de la LENGUA, no del lote, y no existe caso negativo que mostrar. Lo que varía es el cuantificador negativo —niciodată, nimeni, nimic, nicăieri, niciun, nicio: 6 sobre 8—, que es un inventario léxico cerrado y ES la cobertura del punto: el alumno tiene que aprender que la doble negación se aplica a TODOS. Un gate que suspendiera por la señal sola suspendería lengua bien enseñada.',
  ...
  P({ id: 'r3-imperativo-afirmativo', nombre: 'Imperativo afirmativo de 20 verbos frecuentes', bloque: 3, nivel: 'A1',
    ...
    motivo: 'PRIMER PUNTO PUBLICADO CON LA MÁQUINA DE TRANSFORMACIÓN (lote 23, 2026-09-03). No cabe en cloze —un hueco «___ mâine!» admite Vino, Să vii, Hai y Trebuie să vii— y en transformación la consigna ES el ejercicio. TRES COSAS QUE EL DICTAMEN CAMBIÓ Y QUE NO SE REABREN. (1) LA REGLA TIENE TRES PISOS y el orden de evaluación importa: (A) lista cerrada supletiva fă, du-te, adu, zi, vino, fii, más las DEFECTIVAS a putea / a vrea / a trebui / a ști, que no tienen imperativo; (B) CLASE ANTES QUE SINTAXIS: 1.ª conjugación y todo verbo con sufijo -esc/-ăsc toman la 3.ª sg, transitivo o no; (C) sólo en el resto opera transitivo → 3.ª sg / intransitivo → 2.ª sg, y aun ahí gotean a vedea → vezi y a auzi → auzi. La regla de dos pisos «transitivos → 3.ª sg, intransitivos → 2.ª sg» es FALSA y la refutan cinco verbos del propio lexicón (a vorbi, a lucra, a pleca, a intra, a cânta son intransitivos con forma de 3.ª sg): evaluar la transitividad primero da *vorbi! y *lucra! (GALR I, Imperativul; GBLR 2010, Verbul · imperativul; formas en DOOM3 2021 y dexonline). (2) EL PISO (B) ES GRATIS PARA UN HISPANOHABLANTE, y esto invierte el diseño: el imperativo español de 2.ª sg ES formalmente la 3.ª sg del presente (lee = «él lee», canta = «él canta»), así que citește / vorbește / cântă se aciertan traduciendo y miden r3-presente-4-conjugaciones, no este punto. El español NUNCA forma el imperativo con la 2.ª sg del presente, así que el piso (C) —stai!, mergi!, dormi!— es exactamente donde la transferencia falla y produce *stă!, *merge!, *doarme!: son los ítems donde la respuesta correcta es NO tocar la forma, y son el punto, no una concesión. Medido ejecutando la estrategia sobre el lote: «poner la 3.ª sg» acierta 2 de 8. (3) LA FUENTE VA EN LA PERSONA QUE NO COINCIDE CON EL IMPERATIVO —pisos A y B en 2.ª sg, piso C en 3.ª— y NUNCA con să + conjuntivo. La primera versión de esta regla decía «siempre en 2.ª persona», y era media regla: es cierto para A y B, donde la 3.ª regala la respuesta por copia (~22 de los ~40 verbos del lexicón), y FALSO para el piso C, que se define exactamente por «imperativo = 2.ª sg», así que en 2.ª la respuesta queda escrita en la fuente justo en los tres ítems que cargan la regla. Y «Trebuie să vii mâine» contiene literalmente la respuesta calcada, que sale BORRANDO una palabra. LA CONSIGNA TAMBIÉN ES GATE: tiene que atar el LEXEMA («usa el mismo verbo»), porque Du-te la piață! y Hai cu mine! son las formas naturales de esas órdenes —hai la 65 y hai cu mine 13 en el corpus— y un alumno que las escribe escribe mejor rumano que la clave; y tiene que cerrar să por la FORMA («empieza la frase por el verbo») y no por la etiqueta «en imperativo», porque en español el imperativo es funcional antes que morfológico (el negativo, el de usted y el de 1.ª pl. SON subjuntivos) y porque să + conjuntivo es regular sobre todos los verbos: aceptarlo abriría una estrategia que acierta 9 de 9 sin aprender un solo imperativo. Y LO QUE NO SE MARCA COMO MALO, en dos sitios. Să vii! es rumano vivo y normativo (GBLR 2010, conjunctiv cu valoare de imperativ; GALR I como forma supletiva y concurrente) y está atestado en aislamiento en el corpus del proyecto, en el mismo diálogo y del mismo hablante que Vino! («— Vino tu pe la mine. […] — Să vii!»): la consigna lo excluye nombrando la casilla («en imperativo»), no prohibiendo una cadena, y queda escrito para que nadie lo herede como agramatical. LISTA NEGRA medida en el corpus: zi (homógrafo del sustantivo «día», 2.398 apariciones), vezi y auzi (marcadores discursivos gramaticalizados, 1.802 y 922), ia (partícula de atención), du a secas (NO es forma libre: 1 aparición en 2,9 M, y la normativa es du-te), a coborî (cobori atestado 23 veces frente a la norma coboară de DOOM3), y hai/haide, que es interjección y no imperativo de ningún paradigma. fă sólo con objeto, porque es homógrafo de la interjección vocativa dirigida a mujer, atestada en el corpus. Y el SEGUNDO que no se marca como malo: el imperativo rumano SÍ admite sujeto pronominal expreso —tu vino sale 3 veces en el corpus («iar TU VINO cu mine prin ninsori de stele», Eminescu) y pospuesto stai tu 8 y vino tu 3—, sólo que es CONTRASTIVO y no es «esa misma orden»; se excluye pidiéndolo en la consigna («no pongas pronombre»), nunca fingiendo que la lengua no lo tiene. La primera redacción de este punto afirmaba lo contrario y pasó el gate de varianza igual que una verdadera, porque el gate comprueba que el juicio NOMBRE la pieza, no que sea cierto.', cubre: ['A1/GRAMÁTICA · VERBO'],
  ...
  P({ id: 'r4-gd-lui-formula', nombre: 'Genitivo con «lui» ante nombre propio masculino', bloque: 4, nivel: 'A1',
    ...
    varianza: 'LEGÍTIMO, Y ES EL MODELO A IMITAR. Quita «de» y pone «lui» en 7 de 8, y el OCTAVO es su contraejemplo: «Biroul lui domnul Popescu» → «Biroul domnului Popescu», donde la regla NO se aplica porque el nombre común articulado sí porta la marca de caso. O sea que el punto examina las DOS direcciones y el alumno no puede sobregeneralizar «pon siempre lui» y sacar 8/8. Que el umbral sea 80 % y no 100 % es lo que deja ver esto: la pieza invariante y el contraejemplo conviven.',
  ...
  P({ id: 'r4-articulo-posesivo', nombre: 'Artículo posesivo/genitival al / a / ai / ale: CUÁNDO hace falta', bloque: 4, nivel: 'A2',
    descripcion: 'La regla NO es «definido ⇒ no hay artículo»: es de ADYACENCIA. El artículo posesivo falta SÓLO en la configuración [N + enclítico] + [genitivo] pegados (cartea băiatului); en cuanto algo se mete en medio o el núcleo no lleva el enclítico, aparece — y concuerda con lo POSEÍDO, no con el poseedor. Cinco configuraciones productivas en A2, todas con el artículo OBLIGATORIO: núcleo indefinido (o carte a băiatului), núcleo cuantificado (doi prieteni ai Mariei), adjetivo interpuesto con el núcleo DEFINIDO (casa nouă a Mariei), demostrativo pospuesto (mașina aceasta a Mariei) y predicativo con cópula (cartea este a Mariei). Y el enclítico puede estar en otra palabra: noua mașină a lui Ion, cel al Mariei. (GALR I, Articolul posesiv-genitival; Avram, Gramatica pentru toți, cap. Articolul.)',
    prereqs: ['r4-gd-definido-sg'], clase: 'sin-equivalente', calco: { castellano: 'bien', latinComun: 'engañoso' },
    motivo: 'ÚNICO EN LA ROMANIA, y por eso el error diana es la OMISIÓN pura: «un amigo mío», «un libro de María», «dos amigos de María» son español perfecto y no dan ninguna pieza que ocupe ese hueco, así que el alumno produce *un prieten meu, *o carte Mariei, *doi prieteni Mariei. Es SUBPRODUCCIÓN: por eso el formato es transformación y no corrección, y por eso un cloze sería infalsificable (el hueco ya anuncia que falta algo). ⚠ EL PUNTO SE RE-ENCUADRÓ EL 2026-09-04, y el motivo va escrito porque la mitad muerta parece la viva. (1) La mitad FORMA —elegir cuál de las cuatro— está GRATIS por dos rutas que se componen: el atajo de superficie «a + la terminación del artículo enclítico» acierta 138 de 142 celdas del lexicón (medido otra vez tras añadir «perete») (medido, test en paradigma-ro), y el género del español decide bien donde ese falla, porque el español ya concuerda con lo poseído («el/la/los/las DE Juan»); el compuesto de las dos sólo falla donde el enclítico es -le/-ă singular Y la traducción española es femenina, o sea perete/«pared», munte, burete: UN ítem discriminante, no ocho. (2) Y el error «concordarlo con el poseedor», que este mismo motivo declaraba antes, NO lo produce un hispanohablante por esa misma razón: es error de anglohablante o de eslavo (§0.1). Lo que queda, y es propiedad exclusiva de este punto —ninguno de los 8 ítems de r4-gd-lui-formula ni de los 8 de r4-posesivos contiene un solo al/a/ai/ale—, es CUÁNDO hace falta. La mitad forma vuelve entera en B2 con r11-relativo-declinado.', cubre: ['A2/GRAMÁTICA · POSESIVO'],
    // La CITA es el descriptor del currículo, VERBATIM (un test lo exige
    // contra §Rumano), y no se toca al re-encuadrar: dice «elige la
    // forma», que es justamente la mitad que el dictamen del 2026-09-04
    // declaró gratis. Queda escrito aquí para que nadie lea el descriptor
    // como el plan del lote — el plan está en `motivo`.
  ...
  P({ id: 'r4-cel-proforma', nombre: 'DÓNDE se marca la definitud en el grupo del superlativo: prietenul cel mai bun / cel mai bun prieten', bloque: 4, nivel: 'A2',
    descripcion: 'La definitud se marca UNA vez, en el borde izquierdo del grupo nominal (GALR I, «Articolul»; Avram). De ahí las dos caras: POSPUESTO, el nombre conserva su enclítico y ADEMÁS aparece «cel» (prietenul cel mai bun, DOS marcas donde el español y el portugués ponen una); ANTEPUESTO, «cel» carga la definitud y el nombre va DESNUDO (cel mai bun prieten, nunca *cel mai bun prietenul), igual que acest băiat / *acest băiatul.',
    prereqs: ['r4-demostrativos-caso'], clase: 'sin-equivalente', calco: { castellano: 'bien', latinComun: 'opaco' },
    motivo: 'RE-ENCUADRADO EL 2026-09-04 por dictamen del lingüista adversarial, pedido como PRECONDICIÓN y antes de escribir un ítem, y el punto quedó en la CUARTA parte de lo que decía cubrir. (1) EL MOTIVO ANTERIOR ERA FALSO: decía que el hispanohablante produce «*mai bunul» calcando «el más bueno», y no lo produce — el español lleva el artículo DELANTE, así que la transferencia lineal da *mai bun o *cel bun; colgar el enclítico del adjetivo es una operación rumana, de alumno ya avanzado, no un calco de A2. Es la familia del §0.1 por cuarta vez en este inventario. Y «mai-marele» está lexicalizado (DEX s.v. mai-mare; corpus 5), así que tampoco es el modelo. (2) TRES CUARTAS PARTES DEL PUNTO YA ESTABAN COBRADAS, y es la pregunta del §3.1 (¿su mitad útil vive ya en otro sitio?): la CONCORDANCIA de cel/cea/cei/cele la publica entera r8-comparativo, con OCHO ítems, las cuatro formas y la misma trampa del género español que este punto habría usado; «cei doi»/«cele două» lo publica r2-genero-tres-valores con el mismo par doi/două; y la PROFORMA («cel de acolo») es transferencia pura, porque el español tiene «el de allí» y el portugués «o de lá» y este alumno llega con portugués C2 — elegir la forma sólo exige el género, que es otro punto. (3) Y DOS CARAS MÁS NO ESTÁN DETERMINADAS: el artículo adjetival «băiatul cel mic» es OPCIONAL (GALR I; GBLR 2010), así que no hay respuesta única, y la elisión «Cea a lui Ion» tiene tres salidas correctas y CERO apariciones en el corpus del proyecto. (4) LO QUE QUEDA, Y ES LO ÚNICO QUE NO TIENE DUEÑO: dónde se marca la definitud. r2-articulo-enclitico-sg enseña que el enclítico EXISTE y cómo se forma; ningún punto publicado enseña dónde está PROHIBIDO. Por eso el dictamen NO bajó a piso cero: no es una coocurrencia entre dos puntos ajenos, es una regla sin dueño. (5) LOS DOS MODOS DE FALLAR SON DE SIGNO CONTRARIO, y por eso el ítem de sobreaplicación del §0.6 es la segunda cara del propio punto y no un apéndice: en el pospuesto se falla por DEFECTO (*prietenul mai bun, una sola marca, que es la que da el español) y en el antepuesto por EXCESO (*cel mai bun prietenul), y el exceso lo comete precisamente quien acaba de aprender la otra cara. (6) LO QUE NO ENTRA, CON SU MOTIVO: la frontera comparativa (*cel mai bun decât) está RETIRADA porque el cero en que se apoyaba NUNCA SE CORRIÓ — al medirlo, «cel mai X decât» da 1 y «ce(a|i|le) mai X decât» da 8, y al menos dos no admiten lectura de proforma («magazia cea mai deschisă decât toate», «slugile cele mai vechi decât dânsul», las dos con el nombre articulado). GALR I reparte los régimenes (mai + decât/ca; cel mai + din/dintre) pero no marca la cadena como agramatical, así que bajo §0.3 es VIEJA y no MALA. El error fue del lingüista y lo concedió.', cubre: ['A2/GRAMÁTICA · CASO'],
    pisoDeclarado: { piso: 2, motivo: 'DOS ítems, contados por el lingüista adversarial el 2026-09-04 ANTES de escribir ninguno y confirmados en un SEGUNDO ataque que corrigió su propio primer dictamen. El primero daba CINCO, con cuatro ítems de la marca doble (peretele cel mai gros, pâinea cea mai bună, norii cei mai negri, podurile cele mai lungi) más una frontera comparativa. Los cuatro se redujeron a UNO por la pregunta obligatoria del §4.25 —¿qué VARÍA entre los ítems?—: «+cel» y «+mai» son INVARIANTES en los cuatro, así que la marca doble, que es todo el contenido nuevo, se aprende en el primero, y lo único que discrimina del segundo en adelante es la FORMA de cel (que es r8-comparativo, ocho ítems publicados con la misma trampa del género español) y la del ENCLÍTICO (que es r2-articulo-enclitico-sg). Cobertura real 1 y no 4: es r8-relativas-pe-care otra vez, cazado esta vez ANTES de publicar. Y la frontera comparativa cayó porque su cero nunca se corrió (ver `motivo`). Lo que queda son DOS ítems y son de signo contrario: el POSPUESTO, donde el nombre conserva el enclítico y además aparece cel, y el ANTEPUESTO, donde cel carga la definitud y el nombre va desnudo. Con respuesta binaria las dos sobregeneralizaciones opuestas aciertan exactamente la mitad cada una, que es el SUELO teórico y no holgura, así que cargar de un lado subiría una por encima del tope: el punto da para dos y no para más. Los dos ítems son masculino singular A PROPÓSITO —con el género constante la forma de cel es la misma en los dos y no puede discriminar nada, y la POSICIÓN queda como única variable—, y comparten marco, sustantivo y adjetivo, porque a n = 2 cualquier otra diferencia entre las dos fuentes separa las clases al 100 % por construcción.' },
    varianza: 'LEGÍTIMO, y por la misma razón que r3-negacion-antepuesta: las piezas invariantes son «+cel», «+mai» y «-un», en los dos ítems, y la invariancia es de la LENGUA y no del lote: las dos casillas SON superlativos relativos, así que «cel mai» está en las dos por definición, igual que «+nu» en r3-negacion-antepuesta. Lo que varía en su lugar —la presencia o ausencia del enclítico en el nombre, según el adjetivo vaya detrás o delante— pertenece a ESTE punto y a ningún otro. Ver el `pisoDeclarado` para por qué son dos ítems y no cuatro: la versión de cuatro tenía las mismas invariantes y su variable real era la forma de cel, que es r8-comparativo.',
  ...
  P({ id: 'r5-imperativo-negativo', nombre: 'Imperativo negativo: 2.ª sg. con INFINITIVO (nu veni!), 2.ª pl. como el afirmativo (nu veniți!)', bloque: 5, nivel: 'A2',
    ...
    varianza: 'LEGÍTIMO. Pone «nu» en los 8 porque EN RUMANO NO HAY FORMA PROHIBITIVA DE 2.ª PERSONA QUE LO OMITA: la invariancia es propiedad de la LENGUA y no del lote, igual que en r3-negacion-antepuesta, y no existe caso negativo que mostrar. Lo que varía en su lugar, y ES el punto, es la CASILLA de la que sale la forma verbal: infinitivo corto en los cuatro del singular, imperativo AFIRMATIVO plural en los cuatro del plural. Y hay una TERCERA ruta, la que tumbó la primera publicación: «infinitivo corto + ti», que es la primera regla falsa mas la marca de plural del rumano y resuelve LAS DOS casillas — infinitivoCorto+ti da el afirmativo plural en 34 de los 39 verbos del lexicon. Los cuatro plurales son el residuo donde falla (a bea, a avea, a vedea, a coborî), y por aritmetica NINGUN lote de este punto puede llevar un plural donde el atajo funcione: exigir que la ruta del subjuntivo y la compuesta pasen el tope obliga a mitad y mitad con cero atajos acertados. Y la varianza es real y no decorativa porque el alumno tiene UNA ruta de transferencia para cada casilla y ninguna para las dos —la del infinitivo («No fumar» + la forma de cita del diccionario) acierta 4/8, la del subjuntivo (conjuntivo rumano, que coincide con el afirmativo plural en los 39 verbos del lexicón) acierta 4/8—, que en una elección binaria es exactamente el azar: lo único que separa 8 de 4 es haber aprendido de qué depende la casilla. RESERVA ESCRITA, y es el precio del cruce de número: siete de los ocho ítems llevan encima una recuperación de forma que pertenece a r3-irregulares-a1 o a r2-alternancia-vocalica (vino→veni, vezi→vedeți, coboară→coborâți), y ningún ítem las separa. Se acepta porque la confusión de léxico y la de casilla producen respuestas distinguibles (*Nu vino mâine! frente a *Nu vii mâine!), o sea que un fallo no es inatribuible, sólo es caro.',
    motivo: 'MOVIDO A TRANSFORMACIÓN el 2026-09-03, por orden del coordinador y con la medición delante. El punto salió de corrección con 3 de 8: los tres del PLURAL se resuelven traduciendo (el español marca la 2.ª pl. morfológicamente —«no vengáis»— y el rumano «veniți» es a la vez presente, conjuntivo e imperativo, así que traducir da la BUENA), y encima su mala es el error de un GENERADOR que aplica «negativo = infinitivo» sin mirar el número, no de un alumno: nadie escribe el singular teniendo el vocativo plural en su propia frase. Dos más caen en tercera categoría: la mala (*nu vino, *nu zi) es agramatical pero es un error de NATIVO — el hispanohablante no invoca el imperativo afirmativo bajo negación porque su lengua también cambia de forma al negar. Sólo tres eran corrección legítima (nu fii, nu să bei, nu să scrii) y tres no sostienen un punto. En transformación la consigna puede decir «díselo a VARIOS» sin regalar nada, porque allí la consigna ES el ejercicio. Los 8 itemes se retiraron del corpus. CUBIERTO EL 2026-09-03 (lote 24) con 8 itemes de transformación, y el diseño lo decidió la pregunta de qué es GRATIS: el hispanohablante tiene DOS rutas de transferencia y cada una resuelve UNA casilla — la del infinitivo («No fumar» es prohibitivo productivo en español y el infinitivo corto ES la forma de cita del diccionario) acierta el singular, y la del subjuntivo («no vengáis» → conjuntivo rumano, que coincide con el afirmativo plural en los 39 verbos del lexicón) acierta el plural. Ninguna resuelve las dos, así que el lote es 4 y 4 y cada ruta acierta exactamente el azar de una elección binaria. La fuente de los cuatro plurales va en SINGULAR: sin ese cruce el plural se contesta copiando, y con él la desigualdad que hacía imposible conservar el ítem de identidad se rompe. La regla se enuncia «2.ª pl = el AFIRMATIVO plural», NO «= el presente de 2.ª pl»: a fi es el único verbo del rumano que las separa (fiți / sunteți), y NO HAY ÍTEM que lo cubra —a fi es también el único irregular al que el atajo «infinitivo + ti» acierta, así que meterlo subiría esa ruta por encima del tope—: esa mitad se defiende en la LECCIÓN y en esta prosa, con GALR I Verbul · Imperativul. La lección tiene que decir además que la regla sobrevive al clítico (Du-te! → Nu te duce!, el tipo más frecuente del corpus con nu te teme ×77, y la posición del clítico es r6) y que să nu + conjuntivo es una TERCERA forma prohibitiva viva, la única disponible para 1.ª pl y 3.ª: un alumno que sólo vea dos casillas concluirá que Să nu vii! está mal', cubre: ['A2/GRAMÁTICA · IMPERATIVO'],
  ...
  P({ id: 'r6-contracciones-cliticos', nombre: 'El guion del clítico: de qué lado se pega, y dónde NO se pega', bloque: 6, nivel: 'A2',
    descripcion: 'EL PUNTO NO ES DE COLOCACIÓN, ES DE ORTOGRAFÍA, y el nombre y esta línea se reescribieron el 2026-09-04 por eso: la colocación entera es GRATIS para este alumno por dos vías —el español da el orden dativo<acusativo y la proclisis con verbo finito, y el portugués europeo C2 da además que el clítico cuelga del AUXILIAR y nunca del participio (tinha-mo dado) y la idea misma de un clítico pegado con guion (dá-mo, deu-lho)—. Lo que no le da ninguna de las dos: EL GUION RUMANO NO MARCA ENCLISIS COMO EL PORTUGUÉS, MARCA SÍLABA, y por eso «l-am văzut» lo lleva siendo PROCLÍTICO, cosa que el portugués no hace jamás («não me deu», sin guion). De ahí las dos caras que el punto examina: (1) el clítico ASILÁBICO se apoya con guion en la palabra vecina y el SILÁBICO va suelto —«mi-o dă» frente a «mi le dă»—; y (2) por eso mismo «o», que es el único acusativo silábico, es el único que puede ir enclítico a un participio acabado en consonante —«a văzut-o» frente a «l-a văzut», con «a văzut-l» imposible porque la coda «-tl» no existe en rumano—. ⚠ Y NO se llama «obligatorias», que es lo que decía y era falso: con «nu» la contracción es OPCIONAL y así está publicado en r3-negacion-nu, que acepta «nu îl», «nu îmi» y «nu am» como alternativas; con el auxiliar no lo es. ⚠ «într-o» y «dintr-un» SALEN de aquí: no son clíticos sino preposición + artículo (DOOM3 s.v. într-un), estaban en esta línea por listar formas en vez de decir qué se examina, su calco lo bloquea el portugués (num, dum, no, na) y «în o» 76 y «prin un» 63 están atestados, o sea §0.3 viejas y no malas.',
    prereqs: ['r6-cliticos-dativo', 'r3-perfect-compus-intro'], clase: 'sin-equivalente', calco: { castellano: 'bien', latinComun: 'opaco' },
    motivo: 'REENCUADRADO EL 2026-09-04 (lote 28) por el lingüista adversarial, con la pregunta del §3.1 hecha ANTES de escribir un ítem. TRES PUNTOS PUBLICADOS COBRABAN CASILLAS DE ÉSTE, y uno no estaba en ninguna lista: r3-negacion-nu (A1, ocho ítems) publica SEIS contracciones con guion —n-am, nu-l, nu-mi, n-au, nu-i, n-ai—, o sea la casilla «nu ți-l dau» de la cita, y un año antes; r6-doblado-cliticos produce «l-am văzut» y «i-am sunat» Y PUBLICA LA REGLA en su explanationEs («delante de un auxiliar que empieza por vocal se apoya en él: i-am sunat. El guion no es opcional: es la forma que tiene el clítico ahí»); y r3-dativo-experimentante produce «Mi-e foame», «Ți-e frig», «i-e cald» ×5 y de regalo «pune-i o haină» ×2, que es contenido del punto hermano. LA MALA QUE EL MOTIVO DECLARABA («lo he visto» → *îl am văzut) es de un hispanohablante y no de un anglófono —el inglés no tiene clíticos, no hay nada que heredar—, pero está GASTADA por r6-doblado y además es §0.3 VIEJA y no mala: «o am luat» sale en el corpus (Alecsandri) y «îl am» sale 14 veces con límite de palabra aunque los 14 son «a avea» LÉXICO («Îl am cadou», «numa pe el îl am»), ni uno auxiliar. Por eso el formato tiene que ser transformacion y JAMÁS correccion: una tarjeta de corrección marcaría mal rumano atestado. EL EJE DEL ALOMORFO DEL DATIVO ANTE OTRO CLÍTICO (ne→ni, vă→vi, le→li) ES CONTENIDO REAL Y NO ESTÁ PUBLICADO EN NINGUNA PARTE —r6-cliticos-dativo enseña ne, vă, le como LAS formas, ocho ítems, todos sin vecino, así que es el propio currículo el que fabrica el error—, y AUN ASÍ NO ENTRA, porque ninguno de sus dos lados está determinado: «ni-o» sale 2 veces y son clusters reales («nu cum ni-o plănuim noi»; «să găsim pe cineva să ni-o dea la telegraf», Caragiale), «vi-o» sale 9 y también, y «ne le» sale 1 y también («un doctor Gall pipăindu-le, ne le-ar putea spune»). GALR I (Pronumele personal · formele neaccentuate) y DOOM3 dan ni/vi/li en la tabla de distribución pero DESCRIBEN, no proscriben, así que bajo §0 y §0.3 esas formas son raras, no malas, y cualquier clave que las suspendiera suspendería rumano atestado. Se cae el eje entero y no medio: publicar «ni le» sin su frontera enseñaría «reduce siempre el dativo», que es la sobregeneralización que produce *ni-o. ⚠ AVISO DE MÉTODO, porque es el hallazgo más transferible del lote: el primer dictamen mató este eje con una tabla que daba «ne l-a» 30 y «ne le» 86 frente a «ni l-a» 22 y «ni le» 36 —«las dos series atestadas y la no reducida más frecuente»—, y estaba contado con la CLI del corpus, que NO pone límite de palabra, mientras que el gate SÍ lo pone. Con límite: «ne l-a» 1 (y es «Du-ne l-a noastră căsuță», o sea du-ne + la), «ne le» 1, «le-l» 0, «le l-a» 0, frente a «ni-l» 20, «ni le» 20, «vi-l» 17, «vi le» 18. Los 30 y los 86 eran homógrafos: «casne l-au», «cine l-a», «pline L-a», «vine lesne», «tine le fac», «mine leșinată». El eje resucitó, y murió después por la razón buena.', cubre: ['A2/GRAMÁTICA · CLÍTICOS'],
    pisoDeclarado: { piso: 4, motivo: 'CUATRO ítems, contados por el lingüista adversarial el 2026-09-04 ANTES de escribir ninguno y tras TRES ataques, dos de los cuales él mismo perdió. El punto declaraba ocho casillas y de ellas TRES están publicadas bajo otro nombre (nu + clítico y nu + auxiliar en r3-negacion-nu con seis ítems; clítico + auxiliar en r6-doblado-cliticos con dos ítems y la regla escrita en su explanationEs) y LA COLOCACIÓN ENTERA es gratis por dos vías —español y portugués europeo C2—, así que por §4.35 el contenido real es sólo la ortografía del guion. El primer dictamen daba cinco «salidas ortográficas» distintas y TRES DE ELLAS COLAPSAN en una sola cobertura: la regla es la SILABICIDAD (el guion pega al clítico que no puede formar sílaba propia), y una vez aprendida da las tres. Lo que no sale de ella son dos hechos, y cada uno necesita su par mínimo porque la respuesta es binaria y con respuesta binaria la mitad ES el azar: (1) la silabicidad misma, con «mi-o» frente a «mi le» —mismo sujeto, mismo verbo, mismo dativo y EL MISMO sustantivo, cambiando sólo el número del objeto—; y (2) el lado del participio, con «a văzut-o» frente a «l-a văzut» —mismo sujeto, mismo verbo, mismo tiempo, cambiando sólo el género—, que es lo único que el portugués contradice ACTIVAMENTE (tinha-o visto). El quinto y sexto ítem serían el eje del alomorfo, y está muerto por indeterminación de sus DOS lados (ver el motivo). ⚠ La mitad masculina del par 2 («l-a văzut») REPITE material que r6-doblado-cliticos ya publica y está en el lote por el CONTRASTE, no como cobertura: sin ella «pon siempre el clítico detrás del participio» resuelve su mitad al 100 %. Y el límite del lote, escrito para que nadie lo descubra tarde: el par 2 se puede acertar con un PROXY —«si el objeto es femenino, detrás; si no, delante»—, que se ejecuta como estrategia ciega y acierta sus dos; el proxy se rompe en PLURAL (cărțile → le-a văzut, no *văzut-le) y este lote no tiene ese ítem.' },
  ...
  P({ id: 'r6-pe-regla-operativa', nombre: 'Marcador «pe» de objeto directo', bloque: 6, nivel: 'A1',
    ...
    varianza: 'ARREGLADO EL 2026-09-03 (lote 20): el punto vuelve a 8 y ya no aparece marcado, porque examina las DOS direcciones. ERA: quitaba «pe» en los 4 que quedaban, todos del mismo lado de la regla (indefinido no específico: caut un doctor). Los cuatro que enseñaban el lado contrario —«pe» SÍ obligatorio— eran las relativas retiradas, así que el punto no sólo bajó de 8 a 4: quedó DESEQUILIBRADO, enseñando media regla. ARREGLADO con cuatro ítems del lado «SÍ va pe», y el camino hasta ellos es el hallazgo: el «pe» rumano es casi un SUBCONJUNTO de la «a» personal española, así que donde el rumano lo exige el español también marca y el calco ACIERTA — no hay error de calco que corregir por ese lado. Los contextos con divergencia real (Îl vreau PE acesta, «Quiero éste», sin «a» en español) exigen TODOS el doblado clítico y el ítem mediría también r6-doblado-cliticos. Los dos conjuntos son disjuntos. La salida es que el error no es de calco sino de SOBREAPLICACIÓN de la mitad ya enseñada —el alumno omite «pe» donde es obligatorio, y «nimeni» es justo donde lo haría porque PARECE un indefinido no específico— y que con «nimeni» y «cine» el doblado es IMPOSIBLE, no facultativo: «Nu (*l-)am văzut pe nimeni» (Blanca Croitor, Este dublarea complementului direct un fenomen de acord?, Univ. din București 2010, ej. 3). Son los únicos contextos que AÍSLAN «pe». GATE en allowlist con esos dos; «cineva» queda FUERA porque las fuentes discrepan sobre su doblado.',
  ...
  P({ id: 'r7-supin', nombre: 'Supin: mașină de spălat, două cămăși de spălat, nu «vreau de plecat»', bloque: 7, nivel: 'B1',
    descripcion: 'Forma no personal exclusiva del rumano: «de» + participio INVARIABLE con valor de infinitivo/finalidad. La mala NO es «de» + infinitivo corto (*mașină de spăla): eso es agramatical y aun así improducible —hay que borrar la partícula «a», que el español no licencia— y encima es homógrafo de «de» conjuncional + perfecto simple. La mala que un hispanohablante produce de verdad es «de a» + infinitivo (*mașină de a spăla), y sólo es mala AQUÍ: «de a» + infinitivo es rumano correcto y corriente (2.591 apariciones en el corpus del proyecto), pero con nombre CONCRETO sin estructura argumental da cero — con nombre abstracto es lo normal (dorința de a pleca, teama de a greși). Y el supino NO CONCUERDA, a diferencia del participio adjetivo que enseña el prerrequisito r5-participios: «două cămăși de spălat» frente a «două cămăși spălate».',
    prereqs: ['r5-participios'], clase: 'trampa', calco: { castellano: 'bien', latinComun: 'engañoso' },
    motivo: 'corrección desde el calco «de a» + infinitivo, y desde la SOBREAPLICACIÓN en sus dos direcciones (la concordancia que el supino no tiene, y el marco donde el supino no entra). AVISO QUE NO SE PUEDE PERDER: el formato es CIEGO al error dominante. Lo que más hace un hispanohablante de B1 aquí es EVADIR el supino —«am multe să fac», «apă pentru a bea»—, y las dos son rumano CORRECTO: no hay nada que corregir, es subproducción (§0.4/3). El punto no muere por eso, porque quedan dos ejes con mala real, pero un 5/5 aquí NO es dominio del supino y el número no lo dice. MARCOS EXCLUIDOS, cada uno medido en el corpus y no supuesto: (1) el predicativo impersonal que daba nombre al punto —«e greu de crezut»— porque admite «de a» + infinitivo ATESTADO («e ușor de a risca», «nu e greu de a trece pragul vieții», «era lesne de a sfărâma») y una mala arcaica no es una mala; (2) «a avea» + supino, por lo mismo (el corpus trae «aveți de a m[î]nca și de a bea», con la grafía antigua del original) y porque choca con el idioma vivo «a avea de-a face», 23 de cuyas 29 apariciones van bajo «a avea»; (3) «a trebui», porque «trebuie de făcut» es un REGIONALISMO MOLDAVO VIVO y está atestado («Trebuie de înființat numaidecât un monopol»); (4) los lemas a spune/aduce/zice/duce/lua/da en el ítem de concordancia, cuyo participio plural es homógrafo del perfecto simple, así que «de spuse» es lengua correcta («se duse de spuse»). Y una FRONTERA que este formato no puede vigilar y hay que escribir: el compuesto de finalidad NO es predecible —junto a «mașină de spălat» conviven «permis de conducere», «sală de așteptare», «casă de vânzare» con infinitivo largo—, así que la regla es productiva pero el compuesto concreto es léxico. CARAS SIN MALA, comprobadas: el supino nominalizado (la scăldat, spălatul rufelor), el reparto pentru/de, y «a fi» + supino deóntico («ușa e de închis» es correcto y significa «hay que cerrarla», no es un error por «ușa este închisă»).', cubre: ['B1/GRAMÁTICA · FORMAS NO PERSONALES'],
    pisoDeclarado: { piso: 5, motivo: 'CINCO ítems, contados por el lingüista adversarial el 2026-09-03 ANTES de escribir ninguno y con el corpus del proyecto delante. Dos ejes con mala determinada y atestada —«de a» + infinitivo con nombre concreto (×2, uno lexicalizado y otro no, para que el punto no mida sólo el compuesto de diccionario) y la invariabilidad del supino (×2)— más UN ítem de frontera en la otra dirección («Vreau de plecat» → «Vreau să plec»), que es la única sobreaplicación de marco cuyo lema aguanta: con «a trebui» la mala sería regional y con «a fi» no hay mala. Los marcos «e greu de» y «a avea» quedan fuera por variante arcaica ATESTADA en corpus, no por cansancio, y las caras nominalizada, pentru/de y participio-pasivo no tienen mala ninguna. Ocho serían tres clones de los dos ejes.' },
  ...
  P({ id: 'r8-relativas-pe-care', nombre: 'Relativas de OD: «pe care» + reluare clitică, las DOS piezas a la vez', bloque: 8, nivel: 'B1',
    ...
    motivo: 'REINSTANCIADO EL 2026-09-03 tras el dictamen del lingüista adversarial, pedido ANTES de escribir el lote 19 y por orden del coordinador. LA REGLA ES CIERTA Y TIENE FUENTE: «în poziția de complement direct, pronumele care apare obligatoriu însoțit de prepoziția-morfem pe și dublat de un clitic» (Limba Română, Chișinău, nr. 1441; GALR II: 401-403; la cita se transcribe con ș/ț de COMA porque ésa es la norma del proyecto y la del DOOM3 — el original imprime cedilla, que es grafía, no palabra distinta). LO QUE ESTABA MAL ERA LA MALA, y estaba PUBLICADA: los cuatro ítems de este molde (declarados en r6-pe-regla-operativa) ponían «Omul care L-am văzut» —sin pe pero CON clítico—, que no es el calco del español sino la relativa RESUMPTIVA, atestiguada en el rumano coloquial y dialectal con «care» degradado a conector y un pronombre reasuntivo haciendo el papel sintáctico (Blanca Croitor, «Dublarea sintactică în limba română», Limba Română 1/2016, Institutul de Lingvistică al Academiei, p. 7, con ejemplos de corpus y el paralelo del italiano septentrional y el alemán de Suiza), y viva en prensa y TV. Los cuatro corregían habla real y están retirados. Y fallaban además el filtro que ningún gate ve: EL CLÍTICO NO SALE DEL ESPAÑOL — el español no dobla con clítico en la relativa (*«el hombre que lo vi ayer») ni conserva la «a» ante «que» (*«el hombre a que vi»). LA MALA VÁLIDA es sin pe Y SIN clítico —*omul care am văzut ieri—, y su agramaticalidad es de otra naturaleza: no es sub-estándar, es que «care» sólo puede leerse como SUJETO y entonces el verbo no concuerda con el antecedente, así que NO TIENE PARSE. De ahí los dos gates del lote 19: (1) desnudar la buena de «pe» y del clítico tiene que devolver exactamente la mala —invariante estructural, no norma escrita—, lo que de regalo prohíbe la mala resumptiva; y (2) la forma verbal que sigue a «care» se le PREGUNTA a paradigma-ro.ts, porque si coincide con la 3.ª persona que concuerda con el antecedente la mala es rumano correcto (el sincretismo 1.ª sg / 3.ª pl que ya cazó r4-dativo-oi: «Colegii care cunosc» es «los colegas que conocen»). Y EL SEGUNDO ATAQUE, sobre los ocho ítems ya escritos, obligó a reescribirlos ENTEROS antes de publicar, con un hallazgo que vale para cualquier punto de dos piezas: insertar «pe» es INVARIANTE en los ocho —se aprende en el primero y ya no discrimina a nadie—, así que toda la varianza del lote vivía en el CLÍTICO, sobre siete realizaciones (l-, -o, o, i-, le-, îl, le, îi) y TRES subdestrezas: concordancia, elisión ante auxiliar (îl→l-) y enclisis de «o» al participio (am citit-o). Dos de esas tres son de r6-cliticos-acusativo, que ya está cubierto con ocho ítems, y la tercera no está en el inventario en ninguna parte: los ítems medían la ELECCIÓN DEL CLÍTICO, no «pe care», y el fallo se cargaba a este punto. No es reparable dentro del formato, porque «pe care» de OD EXIGE el clítico (*Omul pe care am văzut es agramatical) y la única mala que aislaría «pe» es la resumptiva, que es habla real. La salida es la que da nombre al punto —«pe» y el clítico son UNA sola obligación, y así se enseñan— más bajar la varianza: LOS OCHO ÍTEMS VAN EN PRESENTE, donde sólo quedan las formas plenas îl/o/îi/le y desaparecen la elisión y la enclisis. TERCERA CONDICIÓN DEL MOLDE, y va en gate y no en un comentario porque nadie la recordaría: EL VERBO DE LA PRINCIPAL TIENE QUE IR EN 3.ª PERSONA. La única lectura en que «care» sería sujeto con un verbo de 1.ª es la APOSITIVA a un pronombre de primera («noi, colegii, care așteptăm, suntem obosiți» es correcto), y lo que la bloquea en estos ítems es la persona del principal: el día que alguien la ponga en 1.ª plural, la mala se vuelve gramatical y el ítem muere en silencio. El gate falla ABIERTO donde no puede probar nada —una forma que además es 3.ª plural (sunt, vin, văd, citesc) no demuestra que la cláusula sea de primera y se excluye—, porque si contara toda forma de 1.ª marcaría medio lote por ruido y un gate ruidoso es un gate apagado', cubre: [], sinDescriptor: 'el contenido B1 pide «pe care» obligatorio y ningún «Sabrá hacer» de B1 lo mide',
    varianza: 'ARREGLADO EL 2026-09-03 (lote 20): la varianza ya lo estaba y ahora también el caso negativo. ERA: ponía «pe» en los 8, y lo que varía es el clítico îl/o/îi/le. Eso ERA el defecto (la varianza pertenecía a r6-cliticos-acusativo) y está resuelto por las dos vías que el lingüista pidió: el punto se RENOMBRÓ para que el paquete «pe care + reluare clitică» sea el punto —así la concordancia es suya y no de r6— y los ocho ítems se pasaron a PRESENTE, que elimina la elisión ante auxiliar y la enclisis del participio, dos subdestrezas ajenas. LO QUE SIGUE FALTANDO: ningún ítem muestra la relativa de SUJETO (omul care vine), donde «pe» NO va. Un alumno que aprenda «pon pe care» escribirá *omul pe care vine. ARREGLADO con un ítem de sobreaplicación, y con GATE de plantilla en ALLOWLIST: el antecedente no puede denotar superficie, vía ni vehículo, porque «drumul pe care vine» es CORRECTO en lectura locativa («el camino por el que viene») y la mala dejaría de serlo.',
  ...
  P({ id: 'r8-completivas-ca-sa', nombre: 'Completivas: că vs să, y ca … să con sujeto expreso', bloque: 8, nivel: 'B1',
    descripcion: 'cred că vine / vreau să vină / vreau ca el să vină: indicativo vs conjuntivo, y la partícula «ca» cuando algo se ADELANTA a posición preverbal. CORREGIDO EL 2026-09-03: la regla NO es «ca cuando hay sujeto expreso» — eso describe el caso frecuente, no la condición. «Ca» se licencia por haber un CONSTITUYENTE ADELANTADO delante de «să», sea sujeto o no (Vreau CA PÂNĂ MÂINE să termine Ion cartea, con adverbial adelantado y el sujeto detrás), y un sujeto expreso POSPUESTO no lo pide (Vreau să vină el, sin «ca»). El punto examina esa colocación.',
    prereqs: ['r7-disparadores-sa'], clase: 'trampa', calco: { castellano: 'bien', latinComun: 'transparente' },
    motivo: 'CORREGIDO EL 2026-09-03: el error diana que este punto declaraba —*vreau el să vină— es ERROR DE ANGLÓFONO, no de hispanohablante. Es agramatical (GALR: con material interpuesto entre el regente y `să`, `ca` es obligatorio) pero para producirlo hay que BORRAR el complementante, y el español no lo licencia nunca («*Quiero él venga»); quien lo borra es el anglófono (I want him to come). Es la segunda huella igual en este inventario después de «a asista la» —falso amigo sólo para un anglohablante—, y dos no es coincidencia: hay material heredado de manuales escritos para angloparlantes. LA MALA QUE SÍ PRODUCE UN HISPANOHABLANTE es *Vreau să el vină, con el orden español calcado 1:1: agramatical por la adyacencia să+verbo, donde sólo caben los clíticos pronominales, «nu» y los semiadverbios mai/și/tot/prea/cam. Este punto RECIBE del lote 18 los tres ítems de ese molde, que antes estaban declarados en r7-disparadores-sa: el reparto es que r7 examina qué complementante SELECCIONA el regente y r8 examina la COLOCACIÓN del sujeto. El reparto que/que → că/să coincide casi entero y no se examina. ⚠ LA REGLA ESCRITA DE ESTE PUNTO ERA FALSA, y lo destapó el lingüista el 2026-09-03 atacando el lote 20: «ca aparece cuando hay un sujeto expreso que colocar delante de să» describe el caso FRECUENTE, no la CONDICIÓN. «Ca» se licencia por haber un CONSTITUYENTE ADELANTADO a posición preverbal, sea cual sea su función, y los datos canónicos lo separan del sujeto: «Vreau CA PÂNĂ MÂINE să termine Ion cartea asta» es correcta —lo adelantado es un adverbial y el sujeto Ion va DETRÁS del verbo— mientras que «*Vreau ca până mâine să ION termine cartea asta» es agramatical (Dobrovie-Sorin 1994: 93-95, reproducidos en Cristinel Sava, «Conjuncția compusă» ca (…) să (título transcrito con ț de COMA, norma DOOM3; el original imprime cedilla), Univ. din București 2012, p. 221). Y por el otro lado, «Vreau să vină el mâine» TIENE sujeto expreso y NO lleva «ca». Los ocho ítems publicados son correctos —su mala lo es y su buena también— pero CERTIFICABAN UNA REGLA QUE NO ES LA REGLA, y un alumno que aplicara la vieja produciría *Vreau ca să vină el o rechazaría «Vreau ca mâine să plec», que es correcta. Las ocho explicaciones se reescribieron el 2026-09-03 (los ids cambian porque el hash cubre `data`, y eso se declara en vez de disimularlo). ES LA CUARTA VEZ QUE LA PROSA DEL MATERIAL ENSEÑA ALGO FALSO MIENTRAS LOS ÍTEMS ESTÁN BIEN: arreglar el ítem no arregla la frase que lo explica, y la frase es lo que lee el siguiente', cubre: ['B1/GRAMÁTICA · CONJUNTIVO'],
    varianza: 'ARREGLADO EL 2026-09-03 (lote 20). ERA: ponía «ca» en los 8 y no quita nada más; el regente y el sintagma sujeto varían pero no cambian la operación, así que son contexto y no destreza. El caso negativo EXISTE y no se muestra: «Vreau să vin» (sujeto idéntico, sin ca) y «Vreau să vină el» (sujeto pospuesto, sin ca) — y lo peor es que el pospuesto está declarado como ALTERNATIVA dentro de los propios ítems, o sea que el lote SABE que existe y no lo examina. Un alumno que aprenda «pon ca» escribirá *Vreau ca să vin. ARREGLADO con dos ítems de frontera que piden operaciones DISTINTAS a propósito —si las dos fueran «borra ca», el arreglo repetiría el defecto que viene a arreglar—: uno borra el «ca» que sobra (*Vreau ca să plec) y otro corrige su COLOCACIÓN (*Vreau ca să Ion termine cartea → Vreau ca Ion să termine cartea, Sava 2012 p. 221). Y el primero NO puede llevar ningún adverbial adelantable: con «mâine» dentro, «Vreau CA MÂINE să vin» es gramatical y el ítem tendría dos arreglos, uno de los cuales el corpus no contempla.',
  ...
  P({ id: 'r8-comparativo', nombre: 'Comparativo y superlativo: mai … decât / ca, la fel de … ca, cel mai', bloque: 8, nivel: 'A2',
    descripcion: 'El TÉRMINO DE COMPARACIÓN NO SE EXAMINA: «decât» y «ca» son los DOS el comparativo de superioridad tras «mai» (mai mare decât mine / mai mare ca mine), con «decât» más frecuente y neutro y «ca» corriente y algo menos formal. Medido en el corpus del proyecto, no supuesto: mai+ADJ+decât (contando la grafía antigua en î) = 514 frente a mai+ADJ+ca = 100, en prosa corriente («Tu nu ești mai mare CA mine?», «alta mai frumoasă CA ea»). Un hueco ahí no está determinado. AVISO para quien escriba aquí: «ca» en el comparativo NO es el «como» español de igualdad —la igualdad es «la fel de … ca» / «tot atât de … ca»—, y emparejarlos le enseña al alumno que «ca» marca igualdad. Lo que el punto SÍ examina es (a) la concordancia de cel/cea/cei/cele, con su frontera: «cel» concuerda siempre que haya un NOMBRE al que determinar —incluso cuando lo que «mai» modifica es un adverbio o un participio, «din lecția CEA MAI BINE preparată»— y queda invariable cuando el superlativo modifica al VERBO y no hay nombre («ea cântă cel mai bine»); y (b) el corte bine/bun (vorbește mai bine / mâncarea este mai bună).',
    prereqs: ['r4-cel-proforma'], clase: 'paradigma', calco: { castellano: 'bien', latinComun: 'transparente' },
    motivo: 'cloze de la marca del superlativo, NO del término de comparación. DOS condiciones que se descubrieron a base de tirar ítems y que quien añada aquí tiene que respetar, las dos con gate en scripts/lotes/cloze-ro-l21.ts: (1) el adjetivo tiene que ser INVARIABLE EN GÉNERO (mare, dulce, verde, tare), porque con uno marcado (ieftine, înalți, bună) el alumno copia la desinencia que ya está escrita en la frase y acierta sin mirar el nombre —tres de los cuatro ítems de la v0 se contestaban así, impecables y sin medir nada—; (2) el género ESPAÑOL del nombre tiene que apuntar al revés del rumano (hotelurile «los hoteles» → cele; munții «las montañas» → cei; orașul «la ciudad» → cel; cartea «el libro» → cea), o el ítem se acierta traduciendo. Y sobre el corte bine/bun, dicho porque la primera redacción lo vendía al revés: el español SÍ tiene el corte (bien/bueno) y lo pierde SÓLO en el comparativo, así que el hispanohablante tiene un rodeo infalible —bajar al grado positivo— y el punto mide si lo conoce, no una laguna suya. Sus dos ítems están determinados POR LA PISTA y no por la frase (sin la glosa «mejor» caben mult, des, rar en uno y multă, gustoasă, ieftină en el otro): si alguien acorta o esconde la pista, los dos se vuelven indeterminados a la vez y en silencio (§4.16).', cubre: [], sinDescriptor: 'el contenido A2 pide comparativo y superlativo y ningún «Sabrá hacer» de A2 lo mide',
  ...
  P({ id: 'r8-discurso-indirecto', nombre: 'Discurso indirecto sin consecutio temporum obligatoria', bloque: 8, nivel: 'B1',
    descripcion: 'A spus că vine es la forma no marcada; a spus că venea es gramatical (imperfecto durativo/simultáneo). Lo que el rumano no tiene es el retraso OBLIGATORIO del español. BLOQUEADO: ver «abierto».',
    prereqs: ['r5-perifrasis-pasado'], clase: 'coincide', calco: { castellano: 'mal', latinComun: 'transparente' },
    motivo: 'NO hay formato que pueda examinar este punto hoy, y la descripción de la lengua es correcta: lo que estaba mal era creer que el cloze servía. Ver «abierto» — el piso NO se reduce, la deuda es real.', cubre: [], sinDescriptor: 'contenido B1 sin descriptor propio',
    abierto: 'BLOQUEADO el 2026-09-03 (lote 21), con los dos ítems ESCRITOS, atacados y retirados — no es un punto sin empezar, es un punto medido. Dos cierres independientes: (1) LA ALTERNANCIA vine/venea ES SUBPRODUCCIÓN. Las dos son correctas, el hispanohablante calca el retraso del español y escribe «venea», y cualquier ítem que acepte ambas le da verde SIEMPRE — con razón, porque no hay nada que fallar. Ocho ítems, ocho verdes, cero aprendizaje. Tampoco cabe juicio binario (con una L1 tan cercana la traducción predice la etiqueta) ni traducción (la del español da «venea» y es válida). El ítem que se escribió y se retiró —«Mi-a spus că pe atunci ___ la Cluj, dar acum stă la Iași» → locuia— estaba DETERMINADO y era natural, y aun así se contesta calcando palabra por palabra del español («me dijo que entonces vivía»), y además CONTRADECÍA a la prosa de este mismo punto, que declara esa alternancia no examinable. (2) LA ÚNICA CARA GENUINAMENTE HISPANA —el futuro, donde el español OBLIGA al condicional («dijo que vendría») y el rumano no— tampoco se puede cerrar en un hueco, por dos razones: el rumano tiene TRES futuros vivos más sus variantes (va veni 91 en corpus, o să vină / o să vie 3+21, are să vină / are să vie 7+20, y «are să» + conjuntivo suma 536 apariciones), o sea cinco respuestas correctas para un hueco, que no es un ítem de producción; y sobre todo «ar veni» NO está atestado como agramatical, sólo es pragmáticamente improbable bajo un verbo de promesa — el corpus da 13 casos de «spus/zis/scris que ar…» en valor evidencial-reportativo. Bajo el §0 eso no es una mala. Lo que el punto necesita es un formato de PRODUCCIÓN con consigna («dilo sin retrasar el tiempo»), que no existe en el repo. Y una corrección de nomenclatura para quien lo retome: el condicional rumano es hipotético-optativo y, bajo verbo de decir, evidencial; el PRESUNTIVO es otra forma (o fi venind / ar fi venit) y llamarlo así le enseña al alumno un nombre equivocado.',
  ...
  P({ id: 'r11-cel-complejo', nombre: '«cel» en estructuras complejas y sintagmas densos con doble genitivo', bloque: 11, nivel: 'B2',
    ...
    motivo: 'cadena de concordancias derivable; cloze derivado. ⚠ LO QUE ESTE PUNTO HEREDA DE r4-cel-proforma, escrito el 2026-09-04 al re-encuadrar aquél: su `cita` reclama textualmente «cel de acolo, cel ce, cei doi», y los tres se quedaron SIN cubrir en A2 —«cel de acolo» por transferencia pura (el español da «el de allí» y el portugués «o de lá»), y «cei doi» porque lo publica r2-genero-tres-valores—, así que si alguien los quiere examinar, es AQUÍ y con material de B2, no allí. Y lo que sí sigue vivo y sin publicar en ninguna parte: el GENITIVO-DATIVO de cel (celui 315, celei 152, celor 779 en el corpus, contados con límite de palabra: sin él, `acelui`/`acelor` los inflan) y la casilla supletiva «ceea ce» (1075 frente a «cea ce» 2, DOOM3 s.v. cel), que es la única forma del paradigma que no se deriva.', cubre: ['B2/GRAMÁTICA · RELATIVO'],
  ...
  P({ id: 'r12-dislocacion-cliticos', nombre: 'Dislocación a la izquierda con clítico de recuperación: Cartea, am citit-o ieri', bloque: 12, nivel: 'C1',
    ...
    motivo: '«el libro, lo he leído» transfiere y produce *Cartea, o am citit: la forma del clítico enclítico al participio (citit-o, văzut-o) se corrige desde el calco. ⚠ DOS AVISOS ESCRITOS EL 2026-09-04, del dictamen del lote 28, y el piso NO se toca porque bajarlo no es decisión del agente que los escribe. (1) DUPLICACIÓN §4.20: la enclisis de «o» al participio que este motivo declara como su contenido ES el par 2 de r6-contracciones-cliticos, publicado el 2026-09-04 con «Ana a văzut casa» → «Ana a văzut-o» frente a «Ana a văzut trenul» → «Ana l-a văzut»; y está en la lección b6-l2 como vocabKey, o sea que es A2 y no C1. La dislocación EN SÍ es gratis (es. «el libro, lo he leído»; pt. «o livro, li-o»), así que si la enclisis se la lleva r6 —y se la ha llevado— lo que queda aquí sin dueño hay que volver a contarlo. (2) Y LA MALA QUE ESTE MOTIVO DECLARA ESTÁ ATESTADA: «o a văzut» sale 2 veces en el corpus del proyecto, las dos en el mismo cuento popular transilvano («cum o a văzut bătrâna o a sărutat» — la cita se transcribe con «â» interior porque ésa es la norma del proyecto y la de DOOM3; el original imprime «î», que es grafía y no palabra distinta, igual que se hace con la cedilla en r8-relativas-pe-care), y «o am luat» una (Alecsandri). Bajo §0.3 eso es VIEJO y REGIONAL, no malo, así que el formato de corrección que este punto declara NO se puede usar para *Cartea, o am citit: haría falta transformacion.', cubre: ['C1/GRAMÁTICA'],
