...
export const PUNTOS_LA: PuntoLa[] = [
  ...
  P({ id: 'l1-larga-por-posicion', nombre: 'Larga por naturaleza y larga por posición', bloque: 1, peldano: 'L1',
    ...
    motivo: 'regla derivable. LA CIFRA QUE DECÍA ESTE MOTIVO ERA FALSA y se corrige el 2026-09-12 con la medida: quien sólo mira el mácrón falla en el 7,8 % de las formas atestiguadas de L1 y en el 0,9 % de los tokens, no en la mitad. La confusión probable es con OTRO error, que sí es grande: el instinto castellano de acentuar siempre la penúltima falla en el 32,5 % de las formas y el 9,1 % de los tokens — y ése lo cubre `l1-acento-penultima`, no este punto. Lo que este punto enseña es raro y caro, no frecuente',
  ...
  P({ id: 'l2-primera', nombre: 'Primera declinación: doce celdas y siete formas', bloque: 2, peldano: 'L1',
    descripcion: 'rosa, rosae. Doce celdas y sólo siete cadenas distintas: «-a», «-am», «-ae», «-ā», «-ās», «-ārum», «-īs». La «-ae» sola cubre CUATRO celdas —genitivo singular, dativo singular, nominativo plural y vocativo plural—, así que producirla es fácil y leerla no: resolver cuál de ellas es en cada frase es otro punto, l3-sincretismo.',
    prereqs: ['l2-genitivo-clave'], clase: 'paradigma', formato: 'cloze-derivado',
    calco: { ordenEnganya: 'no', herencia: 'sin-equivalente', via: 'produccion' },
    motivo: 'REENCUADRADO EL 2026-09-11 por el latinista adversarial, con la pregunta de formato hecha ANTES de escribir un ítem. La v0 era `clase: funcion`, `via: recepcion`, con el hueco en la GLOSA «para obligar a decidir la función con el contexto», y ese formato NO PUEDE MEDIR ESO: una glosa española con un hueco fija por construcción qué categoría cabe ahí —«La rosa ___ es hermosa» sólo admite un sintagma con «de»—, así que el alumno lee el marco español, traduce el lema y encaja el determinante sin mirar la desinencia. Techo de la estrategia ciega: 100 %. No es un descuido del ítem: es una propiedad del español, que es configuracional y marca la función con preposición y posición. Es «el suelo que pone la lengua» con el eje invertido —aquí el latín FUSIONA lo que el español DISTINGUE— y de ahí sale un teorema: si la respuesta se escribe en español y el marco español está dado, la función va regalada. Y el intento obvio de arreglarlo se cae solo: dos palabras en «-ae» y dos huecos dejan el ítem INDETERMINADO, porque las dos formas son idénticas y ninguna desinencia dice cuál es cuál. SEGUNDO DEFECTO, INDEPENDIENTE Y PEOR: la v0 invertía un prerrequisito. Este punto es prereq declarado de l3-funcion-por-desinencia, que es EL PUNTO CENTRAL DEL CURSO y el que INSTALA la operación «decidir la función leyendo la forma»; pedirla aquí es exigir en el bloque 2 una destreza que su propio dependiente enseña. Así que el punto vuelve a lo que su nombre dice: es el HERMANO QUE FALTA de l2-segunda, l2-tercera-consonante, l2-tercera-i, l2-cuarta y l2-quinta —los cinco existen como lotes de paradigma y el de la 1.ª era el único ausente—, y el sincretismo se EXHIBE aquí (el alumno escribe «-ae» en cuatro celdas distintas) y se RESUELVE en l3-sincretismo, cuya descripción ya dice literalmente «-ae vale genitivo, dativo y nominativo plural» y cuyo varia es «qué elemento del contexto lo resuelve». LA GEOMETRÍA DEL REPARTO: los lotes de l3 son COLUMNAS —un caso a lo ancho de las cinco declinaciones—, éste es una FILA —una declinación a lo largo de sus seis casos— y l3-sincretismo es la celda donde la fila se pliega sobre sí misma. No había solapamiento: había una tabla que nadie había dibujado entera. ⚠ Y LA V0 CONTABA MAL EL SINCRETISMO: decía «tres funciones» y son CUATRO celdas (faltaba el vocativo plural, atestado —hypocritae ×10, scribae ×6, y filiae Voc.Fem.Plur en Lc 23:28—) más una quinta lectura fuera de la tabla, el locativo singular (Romae). El gate de función iteraba esas tres escritas a mano, así que habría certificado en verde un lote que omitiera el vocativo: un sello responde a UNA pregunta.',
    cubre: ['L1/GRAMÁTICA · DECLINACIÓN'], cita: 'Las cinco declinaciones completas en singular y plural',
    varia: 'la CELDA: doce ítems, uno por casilla del paradigma, para que las cuatro que comparten «-ae» aparezcan como lo que son — cuatro ejercicios distintos con la misma respuesta',
    excepcion: 'el ablativo singular «-ā» sólo se separa del nominativo «-a» por la cantidad, así que es producible y NO es examinable por recepción: el gate de función prohíbe macrones en el marco. Y «fīlia» no puede entrar en dativo ni ablativo plural: la máquina produce «fīliīs», que colisiona con el de «fīlius», y el latín tiene «fīliābus» para deshacerlo (A&G §43.e, atestado ×2) — forma que el paradigma no conoce todavía' }),
  ...
  P({ id: 'l3-nominativo', nombre: 'Nominativo: sujeto y atributo', bloque: 3, peldano: 'L1',
    ...
    calco: { ordenEnganya: 'no', herencia: 'sin-equivalente', via: 'recepcion' },
    motivo: '★ PISO REDUCIDO A 6 EL 2026-09-11, y el número sale de una MEDICIÓN, no de cansancio. El lote tenía 12 y los seis de DOS NOMINATIVOS se retiraron: el eje que decían medir —«con dos, cuál es el sujeto»— no es medible con hueco en la glosa, porque la glosa española nombra ya a uno de los dos y sólo queda una palabra para el hueco. La estrategia ciega «pon el sustantivo que la glosa no menciona» se EJECUTÓ sobre los seis y acertó 6 de 6, sin decidir nada; y no hay desempate semántico que los rescate, porque en español la cópula es reversible. El eje es real —el latín no marca cuál de los dos nominativos es el sujeto y en el texto real el orden acierta el 80,6 %— y vuelve cuando haya formato de dos huecos o de elección múltiple; el gate lo prohíbe mientras tanto. Y un séptimo ítem se ARREGLÓ en vez de retirarse: «Rosas puellae portant» era ambiguo por pro-drop —«llevan las rosas de la niña»— y ahora es «Rosas vident puellae», con la forma al final, donde ningún sustantivo la puede reclamar. Motivo original: con dos nominativos el orden no decide cuál es sujeto — pero en español TAMPOCO («César es general» / «general es César»), así que el instinto posicional no produce aquí ninguna lectura falsa que el español no produzca igual: `ordenEnganya` es «no». Ponerlo en «sí» era ponerlo desde la clase y no desde el error',
