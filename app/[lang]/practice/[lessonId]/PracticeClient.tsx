// app/[lang]/practice/[lessonId]/PracticeClient.tsx
// Client half of the practice route. The server `page.tsx` resolves the
// lesson, pre-renders its MDX (async server component) and hands it down
// as `lessonSlot` — this component can't render the MDX itself (see
// LessonRenderer's header note). Everything else (Dexie session, SRS
// queue, exercise runner) stays client-side as before.
"use client";
  ...
  return (
    <div>
      ...
      {/* L5: LessonGate decides whether to show the lesson (first time
          or last view > 1h ago) or skip straight to practice. The lesson
          MDX is pre-rendered on the server and passed in via `lessonSlot`;
          the gate shows it (wrapped by LessonStep) or the runner — never
          both. */}
      <LessonGate
        lessonId={lesson.id}
        lang={lang}
